...
- `src/mocchiCharacter.ts` exports `createMocchiCharacter()`, returning a narrow `group`, `setMood()`, `triggerTap()`, `setSpeaking()`, `update()`, and `dispose()` interface. The current implementation is procedural low-poly Three.js geometry, so a future GLB-backed factory can replace it without rewriting the UI loop.
